...
import { deleteExistingPatient, fetchPatients } from "../../features/patient/patientSlice";
import { resolvePatientImageUrl } from "../../features/patient/patientService";
...
export default function PatientsTable() {
    ...
    const items = useAppSelector((state) => state.patient.items);
    const total = useAppSelector((state) => state.patient.totalCount);
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            setDeleting(true);
            await dispatch(deleteExistingPatient(selectedId)).unwrap();
            dispatch(
                showAlert({
                    type: "success",
                    message: t("patients.messages.deleteSuccess"),
                })
            );
                ...
                showAlert({
                    ...
                    message:
                        typeof err === "string"
                            ...
                            : err instanceof Error
                                ? err.message
                                : t("patients.messages.deleteError"),
                })
            );
        } finally {
            setDeleting(false);
    ...
    return (
        <>
            ...
            <PageBreadcrumb pageTitle={t("patients.pageTitle")} />

            <div className="space-y-6">
                <ComponentCard title={t("patients.title")}>
                                        ...
                                        items.map((patient) => (
                                            ...
                                            >
                                                <TableCell className="px-5 py-4 text-sm font-semibold">
                                                    <div className="flex items-center gap-3">
                                                        <div className="h-10 w-10 overflow-hidden rounded-full bg-gray-100 dark:bg-white/10">
                                                            <ProfileAvatar
                                                                src={resolvePatientImageUrl(patient.profilePictureUrl)}
                                                ...
                                                <TableCell className="px-5 py-4 text-sm">
                                                    {t(`patients.gender.${patient.genderCode}`, {
                                                        defaultValue: patient.genderCode,
                                                    })}
                                                </TableCell>
                                                <TableCell className="px-5 py-4 text-sm">
                                                ...
                                                <TableCell className="px-5 py-4">
                                                    <Badge size="sm" color={statusColor(patient.currentStatusCode)}>
                                                        {t(`patients.status.${patient.currentStatusCode}`, {
                                                            defaultValue: patient.currentStatusCode,
                                                        })}
                                                    </Badge>
                                                </TableCell>
            ...
            <ConfirmDialog
                isOpen={confirmOpen}
                title={t("patients.messages.deleteTitle")}
                description={t("patients.messages.deleteDescription")}
