...
public class RoleService : IRoleService
{
    ...
    public async Task<Result<Role>> CreateRoleAsync(string roleName, string? description, CancellationToken cancellationToken)
    {
        ...
        if (await _roleManager.RoleExistsAsync(roleName))
        {
            ...
            return Result<Role>.Fail("Role already exists", HttpStatusCode.Conflict);
        }

        var role = Role.Create(Guid.NewGuid(), roleName, description);
        var identityRole = AspIdentityRole.FromDomain(role);
        ...
        if (!result.Succeeded)
        {
            ...
            return Result<Role>.Fail("Creation failed", HttpStatusCode.BadRequest);
        }

        _logger.LogInformation("Role created successfully. RoleId={RoleId}, RoleName={RoleName}", identityRole.Id, roleName);

        return Result<Role>.Created(identityRole.ToDomain());
    }

    public async Task<Result<Role>> UpdateRoleAsync(Guid roleId, string roleName, string? description, CancellationToken cancellationToken)
    {
        ...
        if (role == null)
        {
            ...
            return Result<Role>.Fail("Role not found.", HttpStatusCode.NotFound);
        }
        ...
        if (!identityResult.Succeeded)
        {
            ...
            return Result<Role>.Fail(error, HttpStatusCode.BadRequest);
        }

        _logger.LogInformation("Role updated successfully. RoleId={RoleId}", roleId);

        return Result<Role>.Ok(role.ToDomain());
    }
    ...
    public async Task<Result<PaginatedResult<RoleResponse>>> GetRolesAsync(int pageIndex, int pageSize, string? roleName, CancellationToken cancellationToken)
    {
        _logger.LogInformation(
        ...
        var items = new List<RoleResponse>(roles.Count);

        foreach (var role in roles)
        {
            var users = await _userManager.GetUsersInRoleAsync(role.Name!);

            items.Add(new RoleResponse
            {
                Id = role.Id,
                Name = role.Name!,
        ...
        return Result<PaginatedResult<RoleResponse>>.Ok(
            new PaginatedResult<RoleResponse>(
                pageIndex,
                pageSize,
                totalCount,
                items));
    ...
    public async Task<Result<Role>> GetByIdAsync(Guid roleId, CancellationToken cancellationToken)
    {
        _logger.LogInformation(
    ...
    public async Task<Result<List<Role>>> UpdateUserRolesAsync(Guid userId, IList<string> roles, CancellationToken cancellationToken)
    {
        _logger.LogInformation(
        ...
        var response = updatedRoles.Select(role => role.ToDomain()).ToList();
