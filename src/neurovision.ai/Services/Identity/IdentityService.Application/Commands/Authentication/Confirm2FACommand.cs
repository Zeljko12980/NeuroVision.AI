...
public class Confirm2FACommandHandler : ICommandHandler<Confirm2FACommand, Result<Confirm2FAResponse>>
{
    ...
    public async Task<Result<Confirm2FAResponse>> Handle(Confirm2FACommand command, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Confirm 2FA started. Email={Email}", email);
        ...
        if (!verified)
        {
            _logger.LogWarning("Confirm 2FA failed. Invalid or expired code. Email={Email}", email);
        ...
        if (roles is null)
        {
            _logger.LogWarning("Confirm 2FA failed. Roles not found. Email={Email}", email);
        ...
        {
            _logger.LogWarning("Confirm 2FA failed. User not found. Email={Email}", email);
        ...
        _logger.LogInformation("Confirm 2FA succeeded. Email={Email}", email);
