...
internal sealed class LocationReadStore<TResponse> : ILocationReadStore<TResponse>
{
    private readonly ISqlQueryExecutor _sql;
    private readonly ILocationSql<TResponse> _queries;

    public LocationReadStore(ISqlQueryExecutor sql, ILocationSql<TResponse> queries)
    {
        _sql = sql;
        _queries = queries;
    }
